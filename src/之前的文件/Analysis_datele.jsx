...
class Analysis extends React.Component {
    ...
    matirxRender = (Klabel) => { // 这里的Klabel数据为聚类返回的数据，每个元素都为{时间, 标签值}
        const KLabeldict = {}
        Klabel.forEach((item) => {
        ...
        // 处理出 timeInfo 里面的mae数据(预测发电量与实际发电量的差值)
        for (let key in data) {
            data[key].forEach((item) => {
                // 再次处理出我们所需的时间数据
        ...
        const yScale = d3.scaleBand(this.keyName, [6, padding.height - 18]).round(true) // .round(): 为true,表示启用取整操作。
